...
- [x] Run the API **without** Docker (optional but valuable): from [`api/`](../api/), `./gradlew bootRun` (requires Java 17). Point [`simulator.py`](../simulator/simulator.py) at `http://localhost:8080` temporarily *or* run only the UI against localhost (see README for hostname rules).
